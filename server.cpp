...
public:
    ...
    string getDisplayContent() const override {
        return content;
    }
};
...
public:
    ...
    void insert(Message* msg) {
        Node* newNode = new Node(msg);   // dynamic memory allocation
        if (head == nullptr) {
            head = tail = newNode;
        } else {
            tail->next = newNode;
...
private:
    ...
    void validateInput(const string& sender, const string& receiver, const string& type, const string& content) {
        if (trim(sender).empty()) {
            throw invalid_argument("Sender name cannot be empty.");
        }
        if (trim(receiver).empty()) {
            throw invalid_argument("Receiver name cannot be empty.");
        }
        if (trim(type).empty()) {
        ...
        if (sender.length() > 50 || receiver.length() > 50) {
            throw invalid_argument("Sender/Receiver name is too long.");
        }
        if (content.length() > 500) {
            throw invalid_argument("Message content too long. Max 500 characters allowed.");
        }
    }
...
public:
    ...
    void addMessage(const string& sender, const string& receiver, const string& type, const string& content) {
        ...
        // Runtime polymorphism through base pointer
        if (type == "text") {
            ...
            throw invalid_argument("Invalid message type. Use text, image, or voice.");
        }
